...
from mail.postgresdb import PostgresDB
...
rsa = MailRSA()
db = PostgresDB('localhost', 'demo', 'demo', 'demo', rsa=rsa)
...
def sendmail(request):
    ...
    if request.method == 'POST':
        ...
        subject = str(request.POST['subject']).strip().replace("'", "''")
        content = request.POST['message'].replace("'", "''")
        if db.send_an_mail(username, toreceiver, subject, content):
            ctx["message"] = "已将消息发送至 " + toreceiver + " ！"
            logger.info("{} send a message to {}".format(username, toreceiver))
        else:
            ctx["message"] = "用户 " + toreceiver + ' 不存在！请检查后重新输入。'
            ctx["input_to"] = toreceiver
            ctx["input_subject"] = subject
            ctx["textarea_message"] = content
...
def config(request):
    ...
    if ctx is None:
        return redirect("/")
    username = ctx["username"]
    friends = db.get_user_friend_list(username)
...
def configadd(request):
    if request.method == 'POST':
        username = request.session.get('username', None)
        newfriend = str(request.POST["newfriend"]).strip()
        if username and newfriend:
            # 判断是否已经存在
            current_friend = db.get_user_friend_set(username)
            if newfriend not in current_friend:
                if db.insert_user_friend(username, newfriend):
                    logger.info("{} added a friend".format(username))
...
def configdelete(request):
    if request.method == 'POST':
        ...
        friendid = request.POST.get('fid', None)
        if username and friendid:
            if db.delete_user_friend(username, int(friendid)):
                logger.info("{} deleted a friend".format(username))
                return JsonResponse({"info": "OK"})
